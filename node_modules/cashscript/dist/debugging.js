...
const applyStackItemTransformations = (element, debugStep, vm) => {
    ...
    const transformationsStartState = {
...
const decodeStackItem = (element, stack) => {
    ...
    if (element.type === PrimitiveType.INT)
        return decodeInt(stackItem);
    if (element.type === PrimitiveType.STRING)
        return decodeString(stackItem);
    return `0x${binToHex(stackItem)}`;
};
const failedFinalVerify = (evaluationResult) => {
...
const calculateCleanupSize = (instructions) => {
...
const verifyFullTransaction = (template) => {
    const placeholderScriptId = Object.keys(template.scripts).find((key) => 'unlocks' in template.scripts[key]);
    const placeholderScenarioId = template.scripts[placeholderScriptId ?? '']?.passes?.[0];
...
const isSignatureCheckWithoutVerify = (instruction) => {
