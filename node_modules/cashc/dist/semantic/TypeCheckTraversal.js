import { PrimitiveType, explicitlyCastable, implicitlyCastable, implicitlyCastableSignature, arrayType, ArrayType, TupleType, BytesType, } from '@cashscript/utils';
import { UnaryOpNode, BinaryOpNode, IdentifierNode, VariableDefinitionNode, RequireNode, IntLiteralNode, } from '../ast/AST.js';
import AstTraversal from '../ast/AstTraversal.js';
import { InvalidParameterTypeError, UnequalTypeError, UnsupportedTypeError, CastTypeError, TypeError, AssignTypeError, ArrayElementError, IndexOutOfBoundsError, TupleAssignmentError, BitshiftBitcountNegativeError, } from '../Errors.js';
...
export default class TypeCheckTraversal extends AstTraversal {
    ...
    visitStatementsRecursively(statements, index = 0) {
        if (index >= statements.length)
            return;
        statements[index] = this.visit(statements[index]);
        if (statements[index] instanceof RequireNode) {
            const narrowings = extractBytesNarrowings(statements[index].expression);
    ...
    visitTupleIndexOp(node) {
        ...
        node.type = node.index === 0 ? node.tuple.type.leftType : node.tuple.type.rightType;
        return node;
    }
    visitSlice(node) {
...
function expectBytes(node, actual) {
...
function expectCompatibleBytesBounds(node, left, right) {
    if (!(left instanceof BytesType) || !(right instanceof BytesType))
        return;
    if (left.bound === undefined || right.bound === undefined)
        return;
    if (left.bound !== right.bound)
        throw new UnequalTypeError(node);
}
...
function inferTupleType(node) {
    ...
    return new TupleType(new BytesType(splitIndex), new BytesType(expressionType.bound - splitIndex));
}
function inferSliceType(node) {
...
const inferPaddedBytesType = (node) => {
...
function executeWithNarrowedTypes(narrowings, fn) {
...
function extractBytesNarrowings(expr) {
...
function extractSingleBytesNarrowing(expr) {
    ...
    if (!(sizeNode.expression instanceof IdentifierNode))
        return undefined;
    const { definition } = sizeNode.expression;
    if (!definition || !(definition.type instanceof BytesType) || definition.type.bound !== undefined)
        return undefined;
...
function matchSizeLiteral(expr) {
...
const isSizeOp = (node) => (node instanceof UnaryOpNode && node.operator === UnaryOperator.SIZE);
