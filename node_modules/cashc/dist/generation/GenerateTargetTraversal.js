...
import { asmToScript, encodeBool, encodeInt, encodeString, Op, PrimitiveType, scriptToAsm, generateSourceMap, PositionHint, BytesType, SourceTagKind, } from '@cashscript/utils';
...
export default class GenerateTargetTraversalWithLocation extends AstTraversal {
    ...
    getStackIndex(value, canBeUndefined) {
    ...
    getMostRecentInstructionPointer() {
    ...
    removeFinalVerifyFromFunction(functionBodyNode) {
    ...
    visitCast(node) {
        node.expression = this.visit(node.expression);
        this.emit(compileCast(node.expression.type, node.type, node.isUnsafe), { location: node.location, positionHint: PositionHint.END });
        this.popFromStack();
        this.pushToStack('(value)');
        return node;
    }
    ...
    visitBinaryOp(node) {
        ...
        this.emit(compileBinaryOp(node.operator, bothOperandsAreNumeric), { location: node.location, positionHint: PositionHint.END });
        this.popFromStack(2);
        this.pushToStack('(value)');
